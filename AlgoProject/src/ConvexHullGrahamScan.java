...
public class ConvexHullGrahamScan extends JPanel {
    ...
    private int numberOfPoints = 0; // Added variable to keep track of the number of points
    ...
    private void calculateConvexHull() {
        numberOfPoints = points.size(); // Update the number of points
        ...
        long executionTime = (endTime - startTime) / 100000;
        if (convexHull.size() == points.size()) {
            System.out.println("Time complexity is O(n^2) as all the points are forming hull");
        }
        else{
            System.out.println("Time Complexity is O(nlogn)");
        }

        JOptionPane.showMessageDialog(null, "Number of points: " + numberOfPoints +
                "\nThe execution time is " + executionTime + " ms", "time", JOptionPane.PLAIN_MESSAGE);
    }
